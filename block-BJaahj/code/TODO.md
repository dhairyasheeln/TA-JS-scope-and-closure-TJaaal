...
function multiMap(arrVals, arrCallbacks) {
  let obj={};
  arrVals.forEach((eleValue)=>{
    let temp=[];
    arrCallbacks.forEach((eleCb)=>{
      temp.push(eleCb(eleValue));
    });
    obj[eleValue]=temp;
  });
  return obj;
...
function objOfMatchesWithArray(array1, array2, callback) {
  ...
  let temp=[];
  array1.forEach((ele,index)=>{
  temp.push(callback.reduce((acc,cv)=>{
      return cv(acc);
    },ele));
...
function schedule(array1,array2) {
  ...
  array1.forEach((fn,index)=>{
    setTimeout(fn,array2[index]*1000);
  })
}
